    ...
    for( indice_tabela in 1:quantidade_de_tabelas_na_pagina ){
      ...
      if(quantidade_de_colunas == 8 ){
        ...
        if(indice_de_relatorio == 5){
          delimitador_inicio <- c( 10                , largura_total-180, largura_total-150 , largura_total -130 , largura_total-81 , largura_total-50  , largura_total-25 , largura_total - 10 )
          delimitador_fim    <- c( largura_total-190 , largura_total-151 , largura_total-131 , largura_total -82 , largura_total-51 , largura_total-26  , largura_total-10 , largura_total + 10 )
        }
        #============================================================
...
for( indice_de_relatorio in c(1,2,5) )
{
  ...
  numero_da_pagina             <- 5
  inicio_da_tabela_desejada    <- " DO ÓLEO DIESEL POR PORTO"
...
dataframe_diesel  <- data.frame( "indice_df" = 1, resultado_relatorios[[1]][[1]]$Tabela[[1]] )
indice_de_relatorio <- 2
for(indice_de_relatorio in c(2,5) ){
  dataframe_diesel  <- full_join( dataframe_diesel, data.frame( "indice_df" = indice_de_relatorio, resultado_relatorios[[indice_de_relatorio]][[1]]$Tabela[[1]] ) )
...
dataframe_diesel$Período <- factor( 
  dataframe_diesel$Período, 
  levels = c(
    "jan/15", "fev/15",
    "mar/15", "abr/15",
    "mai/15", "jun/15",
...
dataframe_diesel             <- dataframe_diesel[ order(dataframe_diesel$Período), ]
dataframe_diesel_ordenado    <- dataframe_diesel[order(dataframe_diesel$Período),]
dataframe_diesel_final       <- dataframe_diesel_ordenado[ -which( duplicated( dataframe_diesel_ordenado$Período) ), ]
